...
void printTouchData() {
  ...
  uint16_t base1 = ((uint16_t)MPR121.getRegister(0x21))<<2;
  uint16_t data0 = (((uint16_t)MPR121.getRegister(0x05))<<8) + MPR121.getRegister(0x04);
  uint16_t data1 = (((uint16_t)MPR121.getRegister(0x0B))<<8) + MPR121.getRegister(0x0A);
  Serial << "0 " << base0 << " -- " << data0 << " || ";
  Serial << "3 " << base1 << " -- " << data1 << endl;
}
