...
boolean Touch::pressed(color index) {
  ...
  mpr121_error_t err = MPR121.getError();
  if (err != NO_ERROR) {
    Serial << "TouchError: " << err << endl;
    MPR121.clearError();
  }
  // capsense
//  MPR121.updateTouchData();
...
void Touch::recalibrate() {
  MPR121.stop();
  MPR121.run();
  Serial << F("Touch: recalibrate!") << endl;
  this->printBaseline();
}
...
void Touch::printBaseline() {
    ...
    for( byte i=0; i<N_COLORS; i++ ) {
      data = MPR121.getBaselineData(sensorIndex[i]);
      Serial << data << " ";
    }
    Serial << endl;
}
