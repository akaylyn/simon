...
void loop() {
  ...
  if (touch.anyPressed()) {
    ...
    Serial << "Got Press: " << button << " filter: " << MPR121.getFilteredData(button) << endl;
...
//        Serial << "Touch detected: baseline (bitshift): " << (MPR121.getBaselineData(button)>>2) <<  " filter (bitshift): " << (MPR121.getFilteredData(button)>>2) << endl;
      //Serial << ".";
      stuckCount++;
      if (stuckCount > 50) {
        MPR121.updateAll();
