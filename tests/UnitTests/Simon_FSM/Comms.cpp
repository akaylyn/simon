...
byte towerColor[N_TOWERS] = {I_RED, I_GRN, I_BLU, I_YEL};
byte towerFire[N_TOWERS] = {I_RED, I_GRN, I_BLU, I_YEL};
...
void towerComms() {
  ...
  // track the number of times useful things happended, like gameplay
  static unsigned long usefulThingsCount =  0;
  static Metro loopTimer(10000);
  ...
  // check loops per 10s
  if ( loopTimer.check() ) {
    Serial << F("Loops/sec= ") << loopCount / 10;
    Serial << F("\tSends/sec= ") << usefulThingsCount / 10;
    Serial << F("\t Sends/Loops=") << usefulThingsCount * 100 / loopCount << F("%") << endl;
    loopCount = 0;
    usefulThingsCount = 0;
    loopTimer.reset();
  } else {
